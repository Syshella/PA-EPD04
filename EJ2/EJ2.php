...
<html>
 ...
 <body class="m-5">
      <?php
         ...
         */
        function getCabeceraInforme($tecnico="") {
            $res = "<table>
                        <tr>
                            <td class=\"font-weight-bold\">Nombre de informe</td>
                            <td class=\"font-italic\"> &emsp; Informe de " . $_POST["tipoRegistro"] . "</td>
                        </tr>
                        <tr>
            ...
            if(strcmp($tecnico, "0")!=0){
                $res .= "<tr>
                            <td class=\"font-weight-bold\">Tipo de agrupaci&oacute;n</td>
                            <td class=\"font-italic font-weight-bold\">&emsp; T&eacute;cnico - " . $tecnico . "</td>
                        </tr>";
            }
        ...
        /***
         *  Funcion que genera la tabla a mostrar en el informe
         */
        function getTablaInforme($arrayData) {
            $tiempoTotal = 0;
            $costeTotal = 0;
            ...
            $res .= " 
                     <table>
                         ...
                         <tbody>";
            // Recorre el array con los datos por fila
        ...
        /***
         * Funcion que agrupa los datos de *concepto* por tecnico si la agrupacion por tecnico es elegida
         */
        function generateReport($agrupacion){
            // Separa cada fila en un array
            $arrayData = explode("\n", trim($_POST["conceptos"]));
            $array = [];
            // Genera un diccionario con key por cada tecnico
            $dic = [];
            if(strcmp($agrupacion, "noAgrupar")==0){
                $dic[0] = $arrayData;
            }else if(strcmp($agrupacion, "tecnico")==0){
                for ($i = 0; $i < count($arrayData); $i++){
                    $arrayRow = explode(";", trim($arrayData[$i]));
                    // Comprueba si existe la clave en el diccionario para actualizarla y sino la anade
                    if (array_key_exists($arrayRow[2], $dic)){
                        $content = $dic[$arrayRow[2]];
                        array_push($content, $arrayData[$i]);
                        $dic[$arrayRow[2]] = $content;
                    }else {
                          $dic[$arrayRow[2]] = [$arrayData[$i]];
                    }
                }
            }
            
            // Itera por agrupacion
            foreach($dic as $key => $value){
                echo "<br><br><h3>Informe " . $key . "</h3>";
                $res = getCabeceraInforme($key);
                // Comprobar caso en que no tiene ningun registro para el tipo de informe
                $numRegistros = 0;
                for ($i = 0; $i < count($value); $i++) {
                    // Deshacemos la fila en un array separado por ;
                    $arrayRow = explode(";", trim($value[$i]));
                    if(strcmp(trim($arrayRow[1]), $_POST["tipoRegistro"])==0){
                        $numRegistros += 1;
                    }
                }
                if($numRegistros==0){
                    $res .= "<p class=\"font-weight-bold\"> NO EXISTEN REGISTROS PARA ESTA CONFIGURACI&Oacute;N";
                    $res .= "<br><hr>";
                }else{
                    $res .= getTablaInforme($value);
                }
                echo $res;
            }
        }
...
Taller 2; Pintura ; Pablo N. J. ; tiempo estimado ; 0:30</textarea>
            <br>
            <label for="agrupar">Agrupar</label>
            <select name="agrupar" id="agrupar">
              <option value="noAgrupar">No agrupar</option>
        ...
        if (isset($_POST["generarInforme"])) {
            ...
            if (isset($_POST["nombre"]) && isset($_POST["fecha"]) && isset($_POST["tipoRegistro"]) && isset($_POST["costeHora"]) && isset($_POST["costeTarifa"]) && isset($_POST["agrupar"])) {
                
                generateReport($_POST["agrupar"]);
